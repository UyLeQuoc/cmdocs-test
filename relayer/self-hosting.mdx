...
<Accordions>
  <Accordion title="Sidecar fails to start on boot">
    The Rust relayer starts the TypeScript sidecar automatically. If it fails within 15 seconds, the relayer exits. Check that sidecar dependencies are installed:

    ```bash
    cd services/server/scripts && npm ci
    ```
  </Accordion>
  <Accordion title="Health check returns 500">
    Verify PostgreSQL is reachable and `pgvector` is installed:

    ```bash
    psql $DATABASE_URL -c "CREATE EXTENSION IF NOT EXISTS vector;"
    ```
  </Accordion>
  <Accordion title="Mock embeddings in production">
    Without `OPENAI_API_KEY`, the server uses deterministic mock embeddings (hash-based). Set the key for real semantic search:
  ...
  <Accordion title="Connection pool exhausted">
    Default pool sizes: 10 max connections (relayer), 3 max connections (indexer). Increase via `DATABASE_MAX_CONNECTIONS` if needed.
  </Accordion>
</Accordions>
